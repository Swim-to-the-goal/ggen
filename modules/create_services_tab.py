...
def create_services_tab(page: ft.Page, selected_services, on_checkbox_change):
    services = ["mariadb", "mysql", "postgresql", "minio", "redis", "prometheus", "traefik"]
    checkboxes = []

    def handle_checkbox_change(service_name, e):
        if e.control.value:
            selected_services.add(service_name)
        else:
            selected_services.discard(service_name)
        on_checkbox_change(service_name, e)
    ...
    for service in services:
        checkbox = ft.Checkbox(
            ...
            on_change=lambda e, service=service: handle_checkbox_change(service, e),
        )
